...
def alpha_beta(state, MIN, MAX, player, alpha, beta, depth, path=[]):
    ...
    if terminal(state, player):
        return utility(state, depth, player), state, path + [state]
    if depth == 0:
    ...
    if MAX and not MIN:
        ...
        successors = find_possible_moves(state, player)
        # successors_sorted = node_order(successors, player)
        for successor in successors:
        ...
        successors = find_possible_moves(state, turn)
        # successors_sorted = node_order(successors, turn)
        for successor in successors:
...
def utility(state, depth, winner):
    if winner == "red":
